          ...
          server.middlewares.use(async (req, res, next) => {
            if (req.url === "/api/post-tweet" && req.method === "POST") {
              ...
              req.on("end", async () => {
                try {
                  const { sendTweet } =
                    await import("./scripts/twitter-client.js");
            ...
            if (req.url === "/api/save-audit" && req.method === "POST") {
              ...
              req.on("end", async () => {
                try {
                  ...
                  const boeId = String(data.boeId).replace(
                    /[^a-zA-Z0-9_-]/g,
                    "",
                  );
                  ...
                  const seen = new Set();
                  const updatedIndex = [entry, ...currentIndex]
                    .filter((item) => {
                    ...
                    .sort(
                      (a, b) =>
                        new Date(b.fecha_auditoria).getTime() -
                        new Date(a.fecha_auditoria).getTime(),
                    );

                  const newIndexName = `BOE_Audit_Index_${Date.now()}.json`;
                  await fs.writeFile(
                    path.join(reportsDir, newIndexName),
                    JSON.stringify(updatedIndex, null, 2),
                  );
                  await Promise.all(
                    indexFiles.map((f) => fs.unlink(path.join(reportsDir, f))),
                  );
